...
class UserModuleMiddleware
{
    ...
    public function handle($request, Closure $next, $guard = null)
    {
         ...
         * @param Key $key
         */
        $client = entity('UserModule', 'AuthClientEntity')
        ::getAuthClient(
            [
                [
        ...
        $user = entity('UserModule', 'UserEntity')::getUser(
            [
                'param' => 'userId',
                'value' => cryptoJsAesDecrypt(env('APP_KEY'), $credentials->sub)
            ],
            'userVerifyPhoneNumberDate,userVerifyEmailDate'
        );
        ...
        if (!$user->first()) {
            return response()->json([
                'status'  => 'error',
                'code'    => Response::HTTP_FORBIDDEN,
                'message' => 'user phone number or email not verified',
                'data'    => null
            ]);
        }
